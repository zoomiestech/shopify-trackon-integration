...
export const config = {
  ...
  shopify: {
    ...
    authMode:
      process.env.SHOPIFY_AUTH_MODE ||
      "client_credentials",

    scopes: (
      process.env.SHOPIFY_SCOPES ||
      "read_orders,write_orders,read_merchant_managed_fulfillment_orders,write_merchant_managed_fulfillment_orders,write_fulfillments"
    )
      .split(",")
    ...
    syncTrackingMetafields: bool(
      process.env.SHOPIFY_SYNC_TRACKING_METAFIELDS,
      true
    ),
  },
  ...
  trackon: {
    ...
    publicTrackingUrl:
      process.env.TRACKON_PUBLIC_TRACKING_URL ||
      "https://trackon.in/courier-tracking",

    appKey:
      process.env.TRACKON_APP_KEY || "",

    userId:
      process.env.TRACKON_USER_ID || "",

    password:
      process.env.TRACKON_PASSWORD || "",

    customerCode:
      process.env.TRACKON_CUSTOMER_CODE || "",
    ...
    bookingBodyMode:
      (
        process.env.TRACKON_BOOKING_BODY_MODE ||
        "json"
      ).toLowerCase(),

    defaultWeightKg:
      Number(
        process.env.TRACKON_DEFAULT_WEIGHT_KG ||
        1
      ),

    defaultPieces:
      int(
        process.env.TRACKON_DEFAULT_PIECES,
        1
      ),

    timeoutMs:
      int(
        process.env.TRACKON_TIMEOUT_MS,
        15000
      ),
    ...
    pickup: {
      name:
        process.env.PICKUP_NAME || "",

      address:
        process.env.PICKUP_ADDRESS || "",

      city:
        process.env.PICKUP_CITY || "",

      state:
        process.env.PICKUP_STATE || "",

      pincode:
        process.env.PICKUP_PINCODE || "",

      phone:
        process.env.PICKUP_PHONE || "",
    },
  },
};
...
export function validateBaseConfig() {
  ...
  if (!config.shopify.clientId) {
    errors.push("SHOPIFY_CLIENT_ID is missing");
  }

  if (!config.shopify.clientSecret) {
    errors.push("SHOPIFY_CLIENT_SECRET is missing");
  }

  if (
    !config.shopify.shop &&
    config.shopify.authMode === "client_credentials"
  ) {
  ...
  if (
    !config.adminApiKey ||
    config.adminApiKey === "change-this-to-a-long-random-secret"
  ) {
    errors.push("ADMIN_API_KEY should be set to a strong secret");
  }

  if (
    !["client_credentials", "oauth"].includes(
      config.shopify.authMode
    )
  ) {
    errors.push(
      "SHOPIFY_AUTH_MODE must be client_credentials or oauth"
    );
  }
